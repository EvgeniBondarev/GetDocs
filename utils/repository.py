...
class SqliteRepository(AbstractRepository):
    ...
    def full_text_search(self, user_filter: str, max_records: int = 30) -> list[Record]:
...
class PostgresRepository(AbstractRepository):
    ...
    def get_record_by_id(self, record_id: int) -> Record:
        with psycopg2.connect(self.connection_string) as conn:
            cursor = conn.cursor()
            cursor.execute("SELECT * FROM labs_data WHERE id=%s", (record_id,))
            base_record = cursor.fetchone()
            return render_record_model(base_record)
    ...
    def get_records_count(self) -> int:
        with psycopg2.connect(self.connection_string) as conn:
            cursor = conn.cursor()
            cursor.execute("SELECT COUNT(*) FROM labs_data")
            records_count = cursor.fetchone()
            return records_count[0]
    ...
    def get_records_in_range(self, page_number: int, range: int = 20) -> list[Record]:
        ...
        with psycopg2.connect(self.connection_string) as conn:
            cursor = conn.cursor()
            cursor.execute("SELECT * FROM labs_data WHERE id BETWEEN %s AND %s", (lower_bound, upper_bound))
            base_records_in_range = cursor.fetchall()
            records_in_range = [render_record_model(record) for record in base_records_in_range]
    ...
    def full_text_search(self, user_filter: str, max_records: int = 30) -> list[Record]:
        ...
        search_query = """
            SELECT id
            FROM labs_data
            WHERE to_tsvector('russian', description) @@ to_tsquery('russian', %s);
        """
        self.__init_virtual_table()

        with psycopg2.connect(self.connection_string) as connection:
            with connection.cursor() as cursor:
                cursor.execute(search_query, (user_filter,))
                records_id = cursor.fetchall()
    ...
    def __init_virtual_table(self):
        with psycopg2.connect(self.connection_string) as connection:
            with connection.cursor() as cursor:
                cursor.execute("""CREATE INDEX IF NOT EXISTS idx_labs_data_fts ON labs_data USING gin(to_tsvector('russian', description));""")
                connection.commit()